...
class TaskInstanceHistory(BaseModel):
    """
    TaskInstanceHistory
    """ # noqa: E501
    ...
    executor: Optional[StrictStr] = Field(default=None, description="Executor the task is configured to run on or None (which indicates the default executor)  *New in version 2.10.0* ")
    executor_config: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["task_id", "task_display_name", "dag_id", "dag_run_id", "start_date", "end_date", "duration", "state", "try_number", "map_index", "max_tries", "hostname", "unixname", "pool", "pool_slots", "queue", "priority_weight", "operator", "queued_when", "pid", "executor", "executor_config"]
