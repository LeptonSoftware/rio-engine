...
class TaskInstanceHistoryCollection(BaseModel):
    """
    ...
    task_instances_history: Optional[List[TaskInstanceHistory]] = None
    __properties: ClassVar[List[str]] = ["total_entries", "task_instances_history"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in task_instances_history (list)
        _items = []
        if self.task_instances_history:
            for _item_task_instances_history in self.task_instances_history:
                if _item_task_instances_history:
                    _items.append(_item_task_instances_history.to_dict())
            _dict['task_instances_history'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "task_instances_history": [TaskInstanceHistory.from_dict(_item) for _item in obj["task_instances_history"]] if obj.get("task_instances_history") is not None else None
        })
        return _obj
